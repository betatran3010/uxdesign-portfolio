...
import { ArrowLeft, Clock, Wrench, Users, UserCircle2 } from 'lucide-react';
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onBack, onNext }) => {
  ...
  // NEW ID mapping
  const isReposet = project.id === 1;
  const isXplain = project.id === 3;
    ...
    <div className="pt-10 pb-24 w-full animate-[fadeIn_0.5s_ease-out]">
      ...
      {project.contextOverview && (
        <div className={`${theme.paper} p-6 rounded-2xl border-2 ${theme.border} mb-12`}>
          ...
          <div className={`${theme.heading} font-medium leading-relaxed`}>
            {project.contextOverview}
          </div>
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-20 bg-white p-6 md:p-8 rounded-[2rem] border-2 ${theme.border} shadow-sm`}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {project.tools?.map(t => (
              <span key={t} className={`${theme.tagBg} ${theme.tagText} px-2 py-0.5 rounded-md text-xs font-bold`}>
                {t}
              </span>
            ))}
          </div>
      ...
      <div className={`w-full max-w-6xl mx-auto space-y-24 text-lg leading-relaxed ${theme.text}`}>
        {isXplain ? (
          <>
            <section><XplainProblemSection /></section>
            <section><XplainSolutionSection /></section>
            <section><XplainPersonalizationSection /></section>
            <section><XplainTranslationSection /></section>
            <section><XplainReflectionSection /></section>
          </>
        ) : isReposet ? (
